$(document).ready(function(){
  ...
  $.get("/api/devices", function(devices, status){
    ...
    devices.forEach(function(device,index){
      // alert("Data: " + device.name);
      options.append($("<option />").val(device.name).text(device.name));
    });
  }); 
});
...
$(function () {
  ...
  var JobDetailsPaneView = Backbone.View.extend({
    ...
    render: function () {
      ...
      this.$('[data-action=delete-jobs]').removeClass('deleteable').text('Deletar selecionado(s)')
      return this
    },
    ...
    allowDeleteJobs: function () {
      this.$('[data-action=delete-jobs]').addClass('deleteable').text('Confirmar exclusão')
    },
    deleteJobs: function () {
      var selectedJobIds = this.getSelectedJobs().map(function (j) { return j.get('_id') })
  ...
  var JobDetailsView = Backbone.View.extend({
    ...
    allowDeleteJob: function (e) {
      $(e.currentTarget).addClass('deleteable').text('Confirmar exclusão')
    },
  ...
  // CODE NEEDED TO PROGRAM NEW COMMANDS HERE
  // =======================================================================================
  var CreateJobPaneView = Backbone.View.extend({
    ...
    debug: function () {
      alert($('#job-schedule-startTime').val());
    },
    ...
    saveJob: function () {
      ...
      */
      ...
      // devid = "59c2a2c4a2243cc629fd50c6"
      var command_url = '';
      var data = '';
      const devName = $('#select-devices').val();
      if ($("#on_off").is(":checked")){
        // alert('ligar e desligar');
        command_url = 'http://localhost:8080/api/schedule/on_off_once';
        data = JSON.stringify({deviceName: devName,startTime:$('#job-schedule-startTime').val(),endTime:$('#job-schedule-endTime').val()});
      } else if ($("#on").is(":checked")){
        // alert('ligar');
        command_url = 'http://localhost:8080/api/schedule/on_once';
        data = JSON.stringify({deviceName: devName,startTime:$('#job-schedule-startTime').val()});
      } else if ($("#off").is(":checked")){
        // alert('desligar');
        command_url = 'http://localhost:8080/api/schedule/off_once';
        data = JSON.stringify({deviceName: devName,startTime:$('#job-schedule-startTime').val()});
      }
